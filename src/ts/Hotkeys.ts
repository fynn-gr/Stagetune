...
import type { Hotkey, PlaylistItem } from "./Types";
...
export function addHotkey( key: number) {
  ...
  } else if (get(draggingOrigin) == "playlist" && get(currentDragging)?.type == "track") {

    // Dropping Track from Playlist, adding existin to Hotkey
		rmHotkeyForSameTrack();
...
export function rmHotkey(key: number) {
  get(hotkeys)[key-1].track!.hotkey = null;
  hotkeys.update(e => {
    e[key-1].track = null;
    return e;
  })
